...
const App: React.FC = () => {
  ...
  const startListening = useCallback(async () => {
    try {
      ...
      console.error(err);
      setState(prev => ({ ...prev, error: "Microfoon toegang geweigerd of niet beschikbaar.", isRecording: false }));
    }
  }, []);
  ...
  const processAudio = async (base64Audio: string) => {
    try {
      ...
      setState(prev => ({ 
        ...prev, 
        error: "Kon het nummer niet herkennen. Probeer het opnieuw met luidere muziek.", 
        isAnalyzing: false 
      }));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0c] text-white flex flex-col items-center justify-start p-4 md:p-8 font-sans selection:bg-blue-500/30">
      ...
      <header className="w-full max-w-5xl z-10 flex justify-between items-center mb-12">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500 font-bold tracking-[0.2em] uppercase">Powered by Gemini AI</p>
          </div>
        </div>
      </header>
      ...
      <main className="w-full max-w-4xl z-10 flex flex-col items-center">
        {!state.result && !state.isAnalyzing && !state.isRecording && (
          <div className="text-center py-24 animate-in fade-in zoom-in duration-1000">
            ...
            <p className="text-zinc-500 text-lg mb-12 max-w-md mx-auto font-medium">
              De slimste manier om muziek te herkennen en direct om te zetten in kunst en tekst.
            </p>
            <button 
        ...
        {state.isRecording && (
          <div className="w-full max-w-md flex flex-col items-center gap-12 py-12">
            ...
            <div className="text-center">
              ...
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs">Zorg dat de muziek goed hoorbaar is</p>
            </div>
        ...
        {state.isAnalyzing && (
          <div className="w-full max-w-md flex flex-col items-center gap-10 py-12">
            ...
            <div className="text-center">
              <h3 className="text-2xl font-black mb-3">Even denken...</h3>
              <p className="text-zinc-500 animate-pulse font-medium italic">Ik zoek de lyrics en maak een kunstwerk...</p>
            </div>
          </div>
        )}
        ...
        {state.error && (
          <div className="w-full max-w-md p-8 bg-red-500/5 border border-red-500/20 rounded-[2rem] flex flex-col items-center gap-6 text-center shadow-2xl backdrop-blur-sm">
            ...
            <div>
              <p className="font-black text-xl mb-2 tracking-tight">Geen match gevonden</p>
        ...
        {state.result && state.imageUrl && (
          <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-10 animate-in slide-in-from-bottom-12 duration-1000">
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {state.result.spotifyUrl && (
                  <a 
                    href={state.result.spotifyUrl} 
                    target="_blank" 
                    className="flex items-center justify-center gap-3 py-5 bg-[#1DB954] hover:bg-[#1ed760] text-black font-black rounded-3xl transition-all hover:scale-[1.02] active:scale-95"
                  >
                ...
                <a 
                  href={state.result.youtubeUrl || `https://www.youtube.com/results?search_query=${encodeURIComponent(state.result.artist + ' ' + state.result.title)}`}
                  target="_blank" 
                  className="flex items-center justify-center gap-3 py-5 bg-white text-black font-black rounded-3xl transition-all hover:scale-[1.02] active:scale-95"
                >
            ...
            {/* Right Col: Lyrics & Context (7 cols) */}
            <div className="lg:col-span-7 flex flex-col gap-8 h-full">
              <div className="bg-white/[0.03] backdrop-blur-3xl p-10 rounded-[2.5rem] border border-white/5 flex flex-col h-full shadow-2xl">
                <div className="flex items-center justify-between mb-8">
                  ...
                  {state.result.releaseDate && (
                    <span className="text-[10px] font-black text-zinc-500 bg-white/5 px-3 py-1 rounded-full border border-white/5 uppercase">
                      Released: {state.result.releaseDate}
                    </span>
                  )}
                </div>
                
                <div className="flex-1 overflow-y-auto max-h-[400px] pr-4 custom-scrollbar">
                  <p className="text-2xl font-serif leading-relaxed text-zinc-300 italic whitespace-pre-wrap select-none">
                    "{state.result.lyrics}"
                  </p>
                </div>
                ...
                <div className="mt-12 pt-8 border-t border-white/5">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[10px] font-black text-zinc-600 mb-3 uppercase tracking-widest">Gevonden via:</p>
                      <div className="flex flex-wrap gap-2">
                        {state.result.sources.slice(0, 2).map((source, idx) => (
                          <a 
                            key={idx} 
                            href={source.uri} 
                            target="_blank" 
                            className="text-[10px] px-3 py-1.5 bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white rounded-lg transition-all border border-white/5 font-bold"
                          >
                    ...
                    <button 
                      onClick={startListening}
                      className="w-14 h-14 bg-white/5 hover:bg-white text-zinc-400 hover:text-black rounded-2xl flex items-center justify-center transition-all border border-white/10 group shadow-xl"
                      title="Nieuw nummer"
                    >
      ...
      <footer className="mt-auto py-12 text-zinc-700 text-[10px] font-black uppercase tracking-[0.4em] text-center opacity-50">
        AI Music Discovery &bull; 2024 &bull; Visual Engine v2.0
      </footer>
